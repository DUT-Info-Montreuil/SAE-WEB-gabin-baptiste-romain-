    ...
    public function afficher_interface($products, $searchResults, $searchQuery, $pendingOrders, $msgSuccess = null, $msgError = null) {
        ...
        <div id="app-barman" class="pb-12">
            <div class="max-w-7xl mx-auto px-4 py-6">
                ...
                <?php endif; ?>
                <?php if ($msgError): ?>
                    <div class="bg-red-50 text-red-600 p-4 rounded-2xl border-2 border-red-100 mb-6 font-bold text-center">
                ...
                <div v-show="currentTab === 'client'" class="max-w-2xl mx-auto space-y-6">
                    ...
                    <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm text-center">
                        <h3 class="text-lg font-black text-gray-900 mb-4">Scanner un QR Code</h3>
                        <div id="reader" style="width: 100%; min-height: 300px; display: none;" class="rounded-xl overflow-hidden mb-4"></div>
                        <button id="start-btn" onclick="startScanner()" class="w-full py-3 bg-indigo-600 text-white rounded-xl font-bold uppercase text-xs tracking-widest shadow-lg active:scale-95 transition-transform">
                            Lancer le Scan
                        </button>
                    </div>
        ...
        <script>
            ...
            async function onScanSuccess(decodedText) {
                if (html5QrCode) {
                    ...
                    // Le format attendu est email#timestamp
                    // On ne garde que l'email (partie avant le #)
                    const email = decodedText.split('#')[0];
                    window.location.href = `index.php?page=barman&id=<?= $buvetteId ?>&q=${encodeURIComponent(email)}`;
                }
            }
