...
class modele_gestion extends Connection {
    ...
    public function getOrgaInfo($orgaId) {
        ...
        // Calcul du chiffre d'affaires réel via la somme des commandes
    ...
    public function addStockEntry($productId, $quantity, $supplier, $cost, $orgaId) {
        try {
            ...
            // 1. Vérifier si la buvette a assez d'argent
            ...
            // 3. Créer la ligne d'entrée (avec le prix d'achat)
            ...
            // 4. Mettre à jour le stock actuel du produit
            $stmt = self::$db->prepare("UPDATE Produit SET stock_actuel = stock_actuel + ? WHERE id = ?");
            $stmt->execute([$quantity, $productId]);

            // 5. Déduire le coût de la trésorerie de la buvette
